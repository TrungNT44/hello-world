...
function DrugNotInventoriedController($scope, $rootScope, $injector) {
    ...
    this.standardizeReciveData = this.standardizeReciveData || function (response) {
        if (response && response.Data) {
            $scope.NotInventoriedDrugs = response.Data.ThuocModels;
        } else {
    ...
    // fetch dữ liệu từ server khi ấn nút xem
    ...
    $scope.lapPhieuKiemKe = function () {
        ...
        // push ThuocId của các thuốc chưa kiểm kê vào mảng notInventoriedDrugIds
        angular.forEach($scope.NotInventoriedDrugs, function (drug) {
            notInventoriedDrugIds.push(drug.ThuocId);
        });

        // lưu id các thuốc vào biến session để truyền sang màn hình Create (tạo phiếu mới từ các thuốc chưa kiểm kê)
        sessionStorage.setItem('NotInventoriedDrugIds', JSON.stringify(notInventoriedDrugIds));
