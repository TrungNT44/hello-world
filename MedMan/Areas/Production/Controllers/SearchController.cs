...
namespace Med.Web.Areas.Production.Controllers
{
    public class SearchController : BaseController
    {
        ...
        public JsonResult SearchDrugs()
        {
            ...
            {
                var drugStore = WebSessionManager.Instance.CurrentDrugStoreCode;
                if (!string.IsNullOrEmpty(requestParams.searchForDrugStore))
                {
                    drugStore = requestParams.searchForDrugStore;
                }
                var data = MainApp.Instance.GetCacheDrugs(drugStore,
